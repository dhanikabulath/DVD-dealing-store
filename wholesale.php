...
<body>
<div id="content">
  <div id="frame_head">Wholesale Purchase</div>
  ...
  <table width="730" cellpadding="10">
    ...
    if(isset($_POST['chkbtn'])){
        ...
        $cust_name=$_POST['textfield3'];
        $sql ="SELECT * FROM tb_cus WHERE cus_name='$cust_name'";
        $result=mysqli_query($conn,$sql) or die(mysqli_error($conn));
        if(mysqli_num_rows($result)==0) {
            phpAlert("Customer Not found");
        }
    ...
    ?>
      ...
      <tr>
          <form action="wholesale.php" method="post">
          <td>Item No</td>
          <td><input type="number" name="serial_no" id="number" value="<?php echo (isset($item_id))?$item_id:'';?>"></td>
        <td ><label for="textfield3">Customer Name</label></td>
        <td ><input name="textfield3" type="text" id="textfield3" value="<?php echo (isset($cust_name))?$cust_name:'';?>"></td>
    ...
    <tr>
        <td colspan="4"><table width="600" cellpadding="10">
                ...
                if(isset($_POST['add'])){
                    ...
                    $row = mysqli_fetch_array($result);
                    $qua_stock=$row['qua_stock'];
                    $rent_price=$row['who_price'];
                    $total = $rent_price * $qty;
                    echo "<tr>";
                    echo "<td align='center'>" . $row['item_name'] . "</td>";
                    echo "<td align='center'>" . $qty . "</td>";
                    echo "<td align='center'>" . $row['who_price'] . "</td>";
                    echo "<td align='center'>" . $total . "</td>";
                    echo "</tr>";


                    $sql ="INSERT into tb_whole(cust_name,item_id,qty,total) VALUES ('$cust_name',$item_id,$qty,$total)";
                    //echo $sql;
    ...
    <tr>
        ...
        elseif (isset($_POST['button4'])){
            $sql="delete from tb_whole order by inv_id desc limit 1";
            mysqli_query($conn,$sql);
            phpAlert("Order Cancelled");
        }
        ?>
